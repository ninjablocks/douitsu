...
function init( args ) {
  ...
  server.grant(oauth2orize.grant.code(function(client, redirectURI, user, ares, done) {
    if (client.client_type !== 'confidential') { return done(null, false); }
  ...
  server.exchange(oauth2orize.exchange.password(function(client, username, password, scope, done) {
    ...
    clientent.load$({appid:client.appid}, function(err, localClient) {
      ...
      // client_type of 'public' means the secret cannot be held confidential by the client.
      // in this case, we allow the client to use password authentication without validating
      // the secret. this is typically the case for mobile applications where the user trusts
      // the application to carry the credentials (e.g. via the password grant type) but the
      // mobile application can't store the secret in the binary since someone could extract it.
      // see https://tools.ietf.org/html/draft-ietf-oauth-v2-31 for details
      if(localClient.client_type !== 'public' && localClient.secret !== client.secret) {
        return done(null, false);
      }
      //Validate the user
      userent.load$({email:username}, function(err, user) {
        ...
        userpin.verify_password({pass:user.pass, salt:user.salt, proposed:password}, function(err, result) {
          if (err || !result.ok) {
            return done(null, false);
          }
          ...
          accesstokenent.make$({id$:token, userID:user.id, clientID:client.appid, clientName:client.name, type:'application'}).save$(function(err) {
            ...
            // password auth by definition of the client having the full login credentials
            // means a scope of '*' is provided.
            // FIXME: allow clients to restrict the scope for their own safety
            var ent = atscopeent.make$({accesstoken:token, scope_domain:'*', scope_item:'*'});
