...
module.exports = function (args) {
  ...
  function buildClientContext(req, res, next) {
    ...
    if( clientId && clientSecret ) {
      ...
      // public clients can authenticate without a secret for the purposes of some
      // grant types. other grant types will refuse the client completely if it is public.
      clientent.load$({appid:clientId,client_type:'public'},function(err,client){
        if(err) {return next(err);}
        req.user = client;
        next();
      });
      ...
      accesstokenent.load$(access_token,function(err,at){
        ...
        if (at) {
          userent.load$(at.userID, function(err, user){
            if(user) {
              res.send({id:at.userID, email:user.email, nick:user.nick, name:user.name, image:user.image, active:user.active, confirmed:user.confirmed});
            }
            else {
